...
function swipeUpFn ( index ){
	...
	var NowStep = $('.box-content .pageall').eq(Animate_lastIndex);
	var NextStep = $('.box-content .pageall').eq(index ? index : ++ Animate_Index);
...
$( function (){
	...
	*/
	...
	$$('.s8-3').tap( function (){
		...
		$('.s9-1 .name').text(name.val());
		$('.s9-1 .city').text(city.val());
		$('.s9-1 .zg').text(mendian.val());
		$('.s9-1 .tel').text(phone.val());
	...
	$$('.popbox1 .btn1').tap( function (){
		$('.popbox1 , .mask').hide();
		swipeUpFn();
		$('.s3-2').removeClass('hover');
		window.NotCode = false;
	});
	$$('.popbox1 .btn2').tap( function (){
		$('.popbox1 , .mask').hide();
		swipeUpFn(2);
		$('.s3-2').addClass('hover');
		window.NotCode = true;
...
LoadingImg.push('images/popbox3.png');
